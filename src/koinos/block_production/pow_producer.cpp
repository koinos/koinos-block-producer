...
pow_producer::pow_producer( crypto::private_key signing_key,
    ...
    _num_worker_groups( worker_groups )
{
  ...
  for( std::size_t worker_index = 0; worker_index < _num_worker_groups; worker_index++ )
  {
    ...
    LOG( info ) << "Work group " << worker_index << ": [" << start.convert_to< uint256_t >() << ", "
                << end.convert_to< uint256_t >() << "]";
  }
}
  ...
  try
  {
    ...
    do
    {
      *done = false;
      ...
      block.set_id(
        util::converter::as< std::string >( crypto::hash( crypto::multicodec::sha2_256, block.header() ) ) );
      ...
      for( std::size_t worker_index = 0; worker_index < _worker_groups.size(); worker_index++ )
      {
        ...
        boost::asio::post(
          _production_context,
          std::bind( &pow_producer::find_nonce, this, worker_index++, block, target, start, end, nonce, done ) );
      }

      {
        ...
        if( block_is_stale )
        {
          ...
          *done = true;
          boost::asio::post( _production_context,
                             std::bind( &pow_producer::produce, this, boost::system::error_code{} ) );
          return;
        }
      }
      ...
      pow_data.set_recoverable_signature( util::converter::as< std::string >(
...
void pow_producer::find_nonce( std::size_t worker_index,
...
contracts::pow::difficulty_metadata pow_producer::get_difficulty_meta()
{
  ...
  if( resp.has_error() )
  {
    KOINOS_THROW( rpc_failure,
                  "error while retrieving difficulty from the pow contract: ${e}",
                  ( "e", resp.error().message() ) );
  }

  KOINOS_ASSERT( resp.has_read_contract(),
                 rpc_failure,
                 "unexpected RPC response when retrieving difficulty: ${r}",
                 ( "r", resp ) );
...
{
  boost::asio::post( _production_context, std::bind( &pow_producer::produce, this, boost::system::error_code{} ) );
  _update_timer.expires_from_now( hashrate::update_interval + 2'500ms );
  _update_timer.async_wait( std::bind( &pow_producer::display_hashrate, this, std::placeholders::_1 ) );
}
